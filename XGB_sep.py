...
def load_data():
    ...
    same_program=Merged_data['same_program'].fillna("0"),
...
def main(version,lag):
    ...
    features = [col for col in data_encoded.columns if col not in ['traffic', 'visits_app', 'visits_web', 'visits_app_scaled', 'visits_web_scaled', 'traffic_scaled','datetime', lag, 'traffic_lag', 'group_id', 'time_idx']]
    target = version
    ...
    plt.plot(data_encoded['datetime'], predictions, label='Predicted', linestyle='dashed')
    plt.plot(data_encoded['datetime'], predictions_cf, label='Counterfactual', linestyle='dashed')
    plt.xlabel('Datetime')
    plt.ylabel('Traffic')
    plt.legend()
    plt.title('XGBoost Traffic Prediction')
    plt.xlim(pd.Timestamp("2023-11-10 18:00:00"),pd.Timestamp("2023-11-10 20:40:00"))
    plt.show()
