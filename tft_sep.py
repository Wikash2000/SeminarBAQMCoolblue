...
def load_data():
    ...
    Merged_data = Merged_data[['datetime', 'traffic', 'visits_app', 'visits_web', 'commercial', 'indexed_gross_rating_point','channel','position_in_break', 'program_cat_before', 'program_cat_after', 'spotlength']]    
    #fill na with 0 
    ...
    categorical_columns = [
        "hour", "day_of_week", "month", "channel", "commercial","commercial_lag_1", "commercial_lag_2", "commercial_lag_3", "commercial_lag_4", "commercial_lag_5",
        "channel_lag_1", "channel_lag_2", "channel_lag_3", "channel_lag_4", "channel_lag_5",
        "program_cat_before", "program_cat_after", "position_in_break", "spotlength",
        "program_cat_before_lag_1", "program_cat_before_lag_2", "program_cat_before_lag_3", "program_cat_before_lag_4", "program_cat_before_lag_5",
        "program_cat_after_lag_1", "program_cat_after_lag_2", "program_cat_after_lag_3", "program_cat_after_lag_4", "program_cat_after_lag_5",
...
def main(version,lag):
    ...
    true_categoricals = [
        "hour", "day_of_week","month", "commercial",
    ...
    full_val_dataloader = full_val_dataset.to_dataloader(train=False, batch_size=64, num_workers=0, shuffle=False)
    full_dataloader = full_dataset.to_dataloader(train=False, batch_size=64, num_workers=0, shuffle=False)
    full_dataloader_cf = full_dataset_cf.to_dataloader(train=False, batch_size=64, num_workers=0, shuffle=False)
    ...
    # eval_metrics_naive = get_evaluations(test_set[version][10:],test_set[version][9:-1])
    ...
    full_set_fit = tft.predict(full_dataset).numpy()
    cf_full_set_fit = tft.predict(full_dataset_cf).numpy()
    ...
    plt.plot(data["datetime"],data[version], color = "blue", label="Actual Traffic",linestyle = ":")
    
    plt.xlabel("Datetime", fontsize=14)
    plt.ylabel("Traffic", fontsize=14)
    plt.legend(fontsize=12)
    
    plt.xlim(pd.Timestamp("2023-12-22 18:00:00"),pd.Timestamp("2023-12-22 23:00:00"))
    ...
    # # Set zoom range
    # plt.xlim(pd.Timestamp("2023-12-21 18:00:00"),pd.Timestamp("2023-12-21 22:00:00"))
    # # Formatting
